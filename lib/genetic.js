...
var Genetic = Genetic || (function(){
    ...
    var Clone = function(obj) {
        ...
        return obj.clone();//JSON.parse(JSON.stringify(obj));
    };
    ...
    var Select2 = {
        "OverSelection": function(pop) {
            const OverSelectionParam = Math.floor(pop.length*0.3);
            return [pop[Math.floor(Math.random()*OverSelectionParam)].entity, pop[OverSelectionParam + Math.floor(Math.random()*pop.length*0.7)].entity];
        },
        "Tournament2": function(pop) {
            ...
            for (i=0;i<this.configuration.iterations;++i) {
                ...
                while (newPop.length < self.configuration.size) {
                    if (
                        ...
                        for (let i of [0, 1]) {
                            const toAdd = uniqSet.has(children[i].toString()) ? parents[i] : children[i];

                            newPop.push(toAdd);
                            uniqSet.add(toAdd.toString());
                        }
                    } else {
                        let toAdd;
                        do {
                            toAdd = mutateOrNot(self.select1(pop));
                        } while (uniqSet.has(toAdd.toString()))
                        newPop.push(toAdd);
                    }
                }
